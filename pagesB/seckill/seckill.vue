<template>
	<div class='container' style="background-color: #ffffff;">
		<div style='position: relative;'>
			<heads :title='language.seckill.seckill.title' navWhite='true' ishead_w="1"></heads>
			<!-- #ifndef MP -->
...
<!-- 		<div style="width:100%; height: 300rpx; margin: auto; border: 1px solid #F0AD4E;">
			<image src="../../static/img/5623b89e192b33fedf9b77ce53363d4.jpg" mode="" style="width:100%; height: 300rpx;"></image>
		</div> -->
		<!-- #ifdef MP -->
		...
		<!-- #endif -->
		 <scroll-view scroll-x="true" style="background: #666666;" :scroll-into-view="scroll_index" scroll-with-animation="true">
            <ul class='title_list' :style="'width:'+(20*t_list.length || 100)+'vw'">
                ...
                <li :id="'scroll_' + index" @tap='_change(index,item)' :class="{'active':index == t_index}"
                    ...
                    <span v-if='item.type == 1' class='new_1'>{{language.seckill.seckill.Panic[0]}}</span>
                    <span v-else-if='item.type == 2' class='new_1'>{{language.seckill.seckill.Panic[1]}}</span>
                    <span v-else-if='item.type == 0' class='new_1'>{{language.seckill.seckill.Panic[2]}}</span>
                </li>
            </ul>
        </scroll-view>
		...
		<view class="content-ul">
			...
			<template v-else>
				<view class="content-ul-li item d-f a-c" v-for="(item,index) in list" :key="index">
					<view class="nullTips" v-if="item.num == 0 && item.type == 1">{{language.seckill.seckill.type[5]}}</view>
					<view class="nullTips" v-if="item.type == 0">{{language.seckill.seckill.Panic[2]}}</view>
					...
					<view class="item-body">
						...
						<view>
							...
							<text class="item-num">
								<text v-if="item.num>0">{{language.seckill.seckill.Robbed}}<text class="red">{{ item.buy_num }}</text>{{language.seckill.seckill.piece}}</text>
						...
						<view class="d-f j-sb a-c">
							...
							<view class="o-btn d-f j-c a-c" v-if="item.type === 1 && item.num != 0" :class="{ jg: item.per == 0  }" @tap="_go_details(item)">
								{{language.seckill.seckill.Panic[3]}}
							</view>
							<view class="o-btn d-f j-c a-c end" v-if="item.num == 0 && item.type!=0" @tap="_go_details(item)">
								{{language.seckill.seckill.Panic[3]}}
							</view>
	...
	export default {
		...
		methods: {
			...
			_go_details(item) {
				...
				uni.navigateTo({
					url: '/pagesB/seckill/seckill_detail?pro_id=' + item.pro_id + '&navType=' + item.type + '&id=' + item.id
				})
			},
			// 选择时段
			_change(index, item) {
