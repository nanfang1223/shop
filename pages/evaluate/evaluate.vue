            ...
            <ul class='evaluate-content' ref='evaluate_content'>
                <li v-for='(item,index) in allevaluat' :key='item.id'>
                    ...
                    <div style="margin-top: 20rpx;">
                        ...
                        <!-- 商家回复 -->
                        <div v-if='item.reply' class="huifu">{{language.evaluate.evaluate.business_huifu}}: 
                            <p :class="{'Open_p':spanIndex.indexOf(index)>-1}" @click="Close(index)">{{item.reply}}</p>
                            <span class="Open" @click="Open(index)" :class="{'disnone':spanIndex.indexOf(index)>-1}">{{language.evaluate.evaluate.unfold}}</span>
                        </div>
                    </div>
