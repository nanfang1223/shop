...
import abcdef from '../../../common/dingyi.js'
...
// window.abcdef = -1;
// 立即购买
export function LaiKeTui_buy_handle(me,val) {
   abcdef.abcdef = val == null ? abcdef.abcdef:val;
    if (me.fastTap) {
        me.fastTap = true
        me.type = 3
        me.$refs.lktAuthorizeComp.handleAfterAuth(me, '../../pages/login/login?landing_code=1', function() {
            ...
            if (me.haveSkuBean) {
				...
				console.log(abcdef.abcdef,"me.haveSkuBean6666666666666666" )
                var product = []
                product.push({
                    pid: me.pro_id
                })
                ...
                me.$req.post({data}).then(res => {
                    ...
                    if (res.code == 200) {
						me.haveSkuBean=""
						// console.log( window.abcdef,"window.abcdef")
						if(abcdef.abcdef==2){
							...
							var url = '../pay/orderDetailsr?product=' + product +
                     		...
                     		console.log(5555,"dadad")
							var url = '../pay/orderDetailsrto?product=' + product +
							    '&isDistribution=' + me.isDistribution +
							    '&canshu=true&returnR=0'
							}
                        if (me.pages == 'pagesB') {		
                ...
                if(me.is_seckill){
                    // 以下为初始化选中
					  console.log(me.is_seckill,"55555")
