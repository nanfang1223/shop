...
export function laikePayThree (fromPage) {
    ...
    if (me.useWallte) {
        if (me.wxPayStatue) {
            ...
            uni.showToast({
                title: me.language.toasts.order.laikePayThree[0],
                duration: 1000,
                icon: 'none'
            })
            me.firstFlag = true
        ...
        if (me.wxPayStatue || me.aliPayStatue || me.baidupayStatue) {
            ...
            if (me.afhalen_translateX > 5) {
                if (me.shop_address_id == '') {
                    uni.showToast({
                        title: me.language.toasts.order.laikePayThree[1],
                        duration: 1000,
                        icon: 'none'
                    })
                    me.firstFlag = true
            ...
            uni.request({ //创建订单
                ...
                error: function (err) {
                    ...
                    me.firstFlag = true
                    uni.showToast({
                        title: me.language.toasts.order.laikePayThree[2],
                        duration: 1500,
                        icon: 'none'
                    })
                    setTimeout(function () {
    ...
    me.$nextTick(function () {
        ...
        if (Number(me.value) > should_pay.toFixed(2)) { //输入金额大于商品金额
            ...
            if (Number(me.value).toFixed(2) > should_pay.toFixed(2) && should_pay.toFixed(2) > 0) {
                ...
                // console.log(should_pay)
                uni.showToast({
                    title: me.language.toasts.order.maxTip,
                    icon: 'none',
                    duration: 1500
                })
            }
        ...
        if (me.afhalen_translateX > 5) {
            if (me.shop_address_id) {
                if (me.useWallte) { //钱包支付
                    if (me.password_status == 0) {
                        ...
                        if (Number(me.needpay) != 0 || Number(heji).toFixed(2) > Number(me.value).toFixed(2)) {
                            if (me.wxPayStatue || me.aliPayStatue || me.baidupayStatue) {
                                ...
                                uni.showToast({
                                    title: me.language.toasts.order.moneyMin,
                                    icon: 'none',
                                    duration: 1500
                                })
                                return false
                    ...
                    //支付宝或微信支付--创建订单
                    uni.showLoading({
                        title: me.language.showLoading.axiospaying,
                        mask: true,
                    })
                    // #ifdef MP-WEIXIN
                    if (me.pay_name == 'jp') {
                ...
                uni.showToast({
                    title: me.language.toasts.order.chooseStore,
                    duration: 1000,
                    icon: 'none'
                })
            }
            ...
            if (me.adds_f) {
                            ...
                            success: (res1) => {
                                ...
                                if (status == 1) { //创建成功
                                    ...
                                    if (res1.data.redis_name != '' && me.seckill) {
                                        uni.showToast({
                                            title: me.language.toasts.order.seckillFail,
                                            duration: 1500,
                                            icon: 'none'
                                        })
                                        setTimeout(function () {
                ...
                uni.showToast({
                    title: me.language.toasts.order.shdz,
                    duration: 1000,
                    icon: 'none'
                })
            }
...
export function laikeChooseCoupon (fromPage, index, id, coupon_status) {
    ...
    if (coupon_status == 1) {
        ...
        uni.showToast({
            title: me.language.toasts.order.notCoupon,
            icon: 'none',
            duration: 1500
        })
    }
